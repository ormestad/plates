...
// First check if user is logging in using barcode
if(isset($_POST['user_hash'])) {
	$USER->login_alt($_POST['user_hash']);
}

if($USER->auth>0) {
	// User logged in, show plate field

	// Check if something have been added in plate field
			...
			if(isset($_POST['submit'])) {
				...
				if($plate_data) {
					...
					if($plate_data['status']=="checked_in") {
						...
						if(isset($_POST['plate_verify'])) {
							if($plate['name']==$_POST['plate_verify']) {
								if(plateCheckOut($plate_data,$USER->data['user_email'])) {
						...
						if(isset($_POST['position'])) {
							if(plateCheckIn($plate_data,$_POST['position'],$USER->data['user_email'])) {
					...
					if(isset($_POST['position'])) {
						...
						$addplate=plateAdd($plate['name'],$_POST['position'],$USER->data['user_email']);
						if(!$addplate['error']) {
				...
				if($plate_data) {
					if(isset($_POST['plate_verify'])) {
						if($plate['name']==$_POST['plate_verify']) {
							if(plateCheckOut($plate_data,$USER->data['user_email'],'return')) {
				...
				if($plate_data) {
					if(isset($_POST['plate_verify'])) {
						if($plate['name']==$_POST['plate_verify']) {
							if(plateCheckOut($plate_data,$USER->data['user_email'],'destroy')) {
	...
	if($plate['name']) {
		...
		// Hidden form field with plate info
	...
	// User not logged in so just show barcode field
	$theform->addText('Please login using your personal barcode');
